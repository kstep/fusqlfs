...
Incremental, output debug info. You can repeat this option up to four times. If
it is mentioned once you will see critical DB errors, two C<-D>s in a line
enable DB warnings, three times make errors show full stack, four times means
show FUSE debug tracing output (really noisy). Default is no debug messages.
...
As an alternative to @-prefix you can use C<--config> option instead to load
configuration from given file, so you can write C<--config filename> instead
of C<@filename>. This is useful to put into shebang line in fusqlfs config
file:

    #!/usr/bin/fusqlfs --config
    --user=root
    --password=pa$$w0rd
    --host=localhost
    --engine=mysql
